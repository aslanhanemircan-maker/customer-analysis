...
def open_settings(event=None):
    callbacks = {
        "on_license_filter": on_license_filter,
        "toggle_search": toggle_search_bar_visibility,
        "toggle_reg_btns": toggle_regression_buttons_visibility
    }
    ...
    show_settings_window(
        ...
        current_regression_line=current_regression_line,
        license_mode_str=license_var.get(),  # <--- Bu parametreyi ekledik
        callbacks=callbacks
    )
...
def _highlight_matches(prefix: str):
      ...
      for sc, sd in scatter_points:
            ...
            x_col = get_plot_x_col(df, settings_state, license_var.get())
            xs = []; ys = []
...
def update_fixed_banner():
    """
    Aktif filtreleri yan yana gösterir. Metin oluşturma işi ui_components'e devredildi.
    """
    final_text = get_banner_text(settings_state)
                 ...
                 and (RISK_COL in df.columns):
                  ...
                  if len(sec_df) > 0:
                        ...
                        for _, r in sec_df.iterrows():
                              try:
                                    xv = float(r[get_plot_x_col(df, settings_state, license_var.get())])
                              except Exception:
